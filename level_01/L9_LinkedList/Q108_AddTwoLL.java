...
public class Q108_AddTwoLL {
    public static class Node {
      ...
      public static class LinkedList {
        ...
        private static int carryCalc(Node one, Node two, int p1, int p2, LinkedList res){
          if(p1==0 && p2==0) return 0;
    
          int data=0;
          int carry=0;
          if(p1>p2){
            carry=carryCalc(one.next, two, p1-1, p2, res);
            data=carry+one.data;
          }else if(p2>p1){
            carry=carryCalc(one, two.next, p1, p2-1, res);
            data=carry+two.data;
          }else {
            carry=carryCalc(one.next, two.next, p1-1, p2-1, res);
        ...
        public static LinkedList addTwoLists(LinkedList one, LinkedList two) {
          ...
          int carry=carryCalc(one.head, two.head, one.size, two.size, res);
          if(carry>0){
            res.addFirst(carry);
          }
          return res;
