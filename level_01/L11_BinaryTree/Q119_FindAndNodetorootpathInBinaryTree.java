...
public class Q119_FindAndNodetorootpathInBinaryTree {
    public static class Node {
      ...
      public static ArrayList<Integer> nodeToRootPath(Node node, int data){
        ...
        ArrayList<Integer> lp= nodeToRootPath(node.left, data);
        if(lp.size() > 0){
          lp.add(node.data);
          return lp;
        }
    
        ArrayList<Integer> rp= nodeToRootPath(node.right, data);
        if(rp.size() > 0){
          rp.add(node.data);
          return rp;
        }
