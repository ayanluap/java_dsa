...
public class Q121_PrintNodesKDistanceAway {
    public static class Node {
      ...
      public static void KLevelsDown(Node node, int k, Node blocker){
        ...
        if(node.left!=null) KLevelsDown(node.left, k-1, blocker);
        if(node.right!=null) KLevelsDown(node.right, k-1, blocker);
      }
      ...
      public static ArrayList<Node> FindPath(Node node, int data){
        ...
        if(node.data == data){
          ArrayList<Node> rres= new ArrayList<>();
          rres.add(node);
          return  rres;
        }
    
        ArrayList<Node> lpath= FindPath(node.left, data);
        if(lpath.size() > 0) {
          lpath.add(node);
          return lpath;
        }
    
        ArrayList<Node> rpath= FindPath(node.right, data);
        if(rpath.size() > 0) {
          rpath.add(node);
          return rpath;
        }
